...
const Skills = () => {
  ...
  return (
    <motion.section
      id="skills"
      className="h-screen flex flex-col relative px-3 sm:px-6 pt-8 sm:pt-12 overflow-hidden"
      initial={{ opacity: 0 }}
    ...
    >
      ...
      <div className="relative z-20 flex flex-col h-full">
        ...
        {/* Heading */}
        <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-blue-500 mb-4 sm:mb-6 text-center">
          Skills
        </h2>

        {/* Tab Bar */}
        <div className="flex w-full bg-gray-900 bg-opacity-80 p-1 rounded-xl shadow-md mb-6 sm:mb-10">
          {tabs.map((tab) => (
            <button
              key={tab}
              onClick={() => handleTabChange(tab)}
              className={`relative flex-1 px-2 sm:px-5 py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors duration-200 ${
                activeTab === tab ? "text-white" : "text-gray-400 hover:text-blue-500"
              }`}
            >
              {activeTab === tab && (
                <motion.div
        ...
        {/* Card Carousel */}
        <div className="flex items-center justify-center gap-2 sm:gap-4 w-full flex-1">
          <button
            onClick={() => setCurrentIndex((i) => i - 1)}
            disabled={!canPrev}
            className="p-2 rounded-full bg-gray-900 bg-opacity-80 shadow-md text-blue-500 disabled:opacity-30 transition hover:scale-110 shrink-0"
          >
            ◀
          </button>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {skills.slice(currentIndex, currentIndex + visibleCount).map((skill, index) => (
                <motion.div
                  key={skill.name}
                  className="flex flex-col items-center justify-between bg-gray-900 bg-opacity-80 border border-blue-500 border-opacity-30 rounded-2xl shadow-lg p-4 sm:p-5 md:p-6 w-36 h-48 sm:w-44 sm:h-52 md:w-48 md:h-56"
                ...
                >
                  ...
                  />
                  <p className="text-xs sm:text-sm font-semibold text-gray-200 mt-2 text-center">
                    {skill.name}
                  </p>
        ...
        {/* Dot Indicators */}
        <div className="flex justify-center gap-2 pb-6 sm:pb-10">
